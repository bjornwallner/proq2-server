## $Id: pSW.pm,v 1.12 2001/06/02 21:17:18 peter Exp $
...
BEGIN {
    ...
    if ( $@ ) {
	print $@;
	die("\nSUCKER!!!!! The C-compiled engine for Smith Waterman alignments (Bio::Ext::Align) has not been installed.\n Please read the install the bioperl-ext package\n\n");
	exit(1);
    }
}
...
sub _initialize {
  ...
  #Changed the following..... to above.
...
sub gap {
    ...
    if( defined $val ) {
	if( $val < 0 ) { # Changed to allowd for gap penalty equal to 0 was <= before.
...
sub ext {
    ...
    if( defined $val ) {
	if( $val < 0 ) {  # Changed to allow ext equal to 0
	    $self->throw("Can't have a gap penalty less than 0");
	}
	$self->{'ext'} = $val;
    }
