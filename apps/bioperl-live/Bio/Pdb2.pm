...
=head2 rotate
    ...
    Title   : rotate
    Usage   : Rotates the proteins
    Function: 
    Example : $pdb_obj->rotate
    Returns : 
    Args    : 
...
sub rotate
{
	...
	for(my $i=0;$i<@{$self->{x}};$i++)
	{
	    ...
	    $coords[1][$i]=$self->{y}[$i];
	    $coords[2][$i]=$self->{z}[$i];
	    $rotated_coords[0][$i]=0;
	    $rotated_coords[1][$i]=0;
	    $rotated_coords[2][$i]=0;
	    #print "[$self->{x}[$i],$self->{y}[$i],$self->{z}[$i]]\n";
	    #push(@coords,[$self->{x}[$i],$self->{y}[$i],$self->{z}[$i]]);
	    #push(@rotated_coords,([0,0,0]));
	}
	...
	my @R=([$d_x*(cos($psi)*cos($phi)-cos($theta)*sin($phi)*sin($psi)),
		$d_y*(cos($psi)*sin($phi)+cos($theta)*cos($phi)*sin($psi)),
		$d_z*(sin($psi)*sin($theta))],
	       [$d_x*(-sin($psi)*cos($phi)-cos($theta)*sin($phi)*cos($psi)),
		$d_y*(-sin($psi)*sin($phi)+cos($theta)*cos($phi)*cos($psi)),
		$d_z*(cos($psi)*sin($theta))],
	       [$d_x*(sin($theta)*sin($phi)),
		$d_y*(-sin($theta)*cos($phi)),
		$d_z*(cos($theta))]);
	

	#print "$R[0][0] $R[0][1] $R[1][2]\n$R[1][0] $R[1][1] $R[2][2]\n$R[2][0] $R[2][1] $R[2][2]\n";
	#exit;
		
	#print scalar @{$coords[0]};
	#exit;
	for(my $i=0;$i<3;$i++)
	{
	    for(my $j=0;$j<scalar @{$coords[0]};$j++)
	    {
		for(my $k=0;$k<3;$k++)
		{
		    
		    $rotated_coords[$i][$j]+=$R[$i][$k]*$coords[$k][$j];
		}
	    }
	}
	...
	#print scalar @coords;
	#print "\n";
	#print scalar @rotated_coords;
	
	for(my $i=0;$i<scalar @{$coords[0]};$i++)
	{
	    #print "[$self->{x}[$i],$self->{y}[$i],$self->{z}[$i]] -> [$rotated_coords[0][$i],$rotated_coords[1][$i],$rotated_coords[2][$i]]\n";
	    $self->{x}[$i]=$rotated_coords[0][$i];
	    $self->{y}[$i]=$rotated_coords[1][$i];
	    $self->{z}[$i]=$rotated_coords[2][$i];
	    #print "[$self->{x}[$i],$self->{y}[$i],$self->{z}[$i]] -> [$rotated_coords[$i][0],$rotated_coords[$i][1],$rotated_coords[$i][2]]\n";
    #push(@coords,[$self->{x}[$i],$self->{y}[$i],$self->{z}[$i]]);
	}
	...
	#
	#my @Ry=([cos($rot_theta),0,-sin($rot_theta)],
	#	  [0,1,0],
	#	  [sin($rot_theta),0,cos($rot_theta)]);
	#
	#my @Rz=([cos($rot_psi),sin($rot_psi),0],
	#	  [-sin($rot_psi),cos($rot_psi),0],
	#	  [0,0,1]);
	#
    ...
    {
	print STDERR "Rotation angles for all three axis must be suplied (phi(z),theta(x),psi(z))\n";
	return 0;
...
sub print_ca_pdb
{
   ...
   #SEQRES
	...
	if($ali[$i] ne "-")
	{
	    ...
	    #printf ("%-6s %4d  %-3s%1s%3s %1s%5s    %7.3lf %7.3lf %7.3lf\n", $self->{field}[$self->{resCA}[$i]],$self->{atomno}[$self->{resCA}[$i]],$self->{atomtype}[$self->{resCA}[$i]],$self->{atom_alt_loc}[$self->{resCA}[$i]],$self->{res}[$self->{resCA}[$i]],$chain,$self->{resno}[$self->{resCA}[$i]],$self->{insertion_code}[$self->{resCA}[$i]],$self->{x}[$self->{resCA}[$i]],$self->{y}[$self->{resCA}[$i]],$self->{z}[$self->{resCA}[$i]]);
	}
    }
}
