...
sub _read_pdb 
{
	...
	{
	    if($ali[$i] eq "-")  #if CA not aligned with "first atom" in resdue
	    {
		$resbegin[$i]="-";
		$resCA[$i]="-";
		#print "$resbegin[$i] $resCA[$i]\n";
	    }
	    else  #add pointer (index) stored in the *_temp
	    {
    ...
    my $self={name => $name,
	      ...
	      y => [@y],
	      z => [@z],
	      resbegin => [@resbegin],  #pointer to begin of a residue if == "-" CA is missing for that residues
	      resCA => [@resCA]};        #pointer to CA of a residue if == "-" CA is missing for that residues
	      #table321 => {%aa321}};
