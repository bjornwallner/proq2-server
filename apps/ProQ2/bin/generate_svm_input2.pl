#!/usr/bin/perl -w 
require '/afs/pdc.kth.se/home/b/bjornw/source/perl/stat.pl';
...
#    print $file."\n";
	...
	if($file=~/\.pdb\.fixed$/ ||
	   ...
	   ($relax && $file=~/\.pdb$/)) {
	    ...
	    #print "id:$id\n";
	    if($classify) {
		    $proqoutfile="$file.$proqres_name"; #proqres2";
...
#		`touch $proqoutfile`;
		...
		$proqbinary="/afs/pdc.kth.se/home/b/bjornw/source/c/pdb/ProQres/PROQRES/bin/ProQres64";
		...
		`$proqbinary -pdb $pdbfile -surf $rsa -stride $stride -psipred $ss2 -prof $prof $proqres_flags -output_input > /scratch/tmp.$$`;
		
		`gzip -f /scratch/tmp.$$`;
		`mv tmp.$$ $proqoutfile.gz`;
		#next;
