#!/usr/bin/perl -w 
require '/Library/WebServer/ProQM/bin/stat.pl';
...
#$outdir=$ARGV[1];
$noprefix=0;
if($param{-noprefix}) { #this will ignore the Sscore...
...
#update to new S-score calculations....		$Sscorefile="$dir/S/$file.S.$Sscore_cutoff.gz";
		...
		$Sscorefile="$file.S.gz" if($dir_is_file); 
	    }else {
...
#		    `/afs/pdc.kth.se/home/a/arnee/MODULES/scientific/stride/linux/bin/stride $pdbfile > $stride`;
		...
		$proqbinary="export PROQRESDIR=/Library/WebServer/ProQM/ProQres/weights;/Library/WebServer/ProQM/bin/ProQres";
#		$proqbinary="/afs/pdc.kth.se/home/b/bjornw/source/c/pdb/ProQres/PROQRES/bin/ProQres.Darwin" if(-e "/Users/");
		...
		`$proqbinary -pdb $pdbfile -surf $rsa -stride $stride -psipred $ss2 -prof $prof $proqres_flags -output_input > $proqoutfile`;
		`gzip -f $proqoutfile`;
		#next;
...
#			print $temp[6]."\n";
	...
	#	exit;
	    ...
	    {
		if(!$casp7) {
		    print "$mprap_file does not exist cannot read surface info...\n";
		    next;
		}
	    }
	    if(-e $co_file) {
...
#		$svmout="$outdir/$file.svm.$id2set{$id}.SscoreNorm$Sscore_cutoff";
		...
		#if(!-e $svmout_proqres_pred) 
		{
		    ...
		    open(OUT2, ">$svmout2") || die;
		    open(OUT3, ">$svmout_proqres_pred") || die;
		    open(IN, "gunzip -c $proqoutfile.gz|") || die;
