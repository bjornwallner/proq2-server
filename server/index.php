                      ...
                      <tr>
                        <td valign=\"top\" colspan=\"2\">
                          ...
                          <form method=\"post\" enctype=\"multipart/form-data\">
                           ...
                           <p><input type=\"checkbox\" name=\"nomail\" value=\"Yes\">Do not send an email when the result is done</p> 
...
#	print "folder: $folder";
	$folder=rtrim($folder);
	$temp=preg_split('/\//',$folder);
	$num=count($temp);
	$folder_base_tmp=array_slice($temp,$num-3,$num-1,TRUE);
	$folder_base=join("/",$folder_base_tmp);
#	print $folder;
#	print "<hr>";
#	print $folder_base;
...
#	print "sbatch /local/www/services/ProQ2/apps/ProQ2/bin/ProQ2.pl $folder/input.pdb $folder/email $folder/seqres $bfactorPDB<br>";
...
#	$pred=preg_replace('/\n/','<BR>',$pred);
#	$user_text.=$pred;
	$link="http://duffman.it.liu.se/ProQ2/$folder_base";
	$user_text="An email with will be sent to $email when the job is done.\n";
	if($email=="noemail") {
	    $user_text="No email will be send instead you will have to monitor when the job finishes by checking the link below.\n";
	}
	$user_text.="The job folder is located here: <a href=$link>$link</a>\n";
    }
  else
    {
      $user_heading="Error";
